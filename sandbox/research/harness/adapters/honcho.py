#!/usr/bin/env python3
"""Honcho harness adapter. Simulates directional peer memory + context retrieval."""
...
from honcho_sim import compose_reply, export_character, ingest_peer  # noqa: E402
...
def run(world: Path, artifacts: Path) -> dict[str, int | float]:
    ...
    if active_peer is None:
        raise AdapterNotReadyError("honcho: active character missing from meta")

    reply = compose_reply(active_peer, query)
    characters = {cid_s: export_character(peer) for cid_s, peer in peers.items()}
