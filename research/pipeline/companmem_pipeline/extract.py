...
from companmem_pipeline.paths import Namespace, load_dotenv, parse_namespace, slug_cache
from companmem_pipeline.prompts import (
    LEDGER_KINDS,
    kind_focus,
    prompt_hash,
    prompt_version,
...
PROMPT_VERSION = prompt_version("product")
...
def normalize_extract(parsed: dict[str, object], *, kind: str, url: str) -> dict[str, object]:
    ...
    if isinstance(ledger_in, list):
        for row in ledger_in:
            ...
            if row_kind not in LEDGER_KINDS:
                row_kind = kind
...
def extract_page(
    ...
    *,
    ...
    if not force and meta_path.exists() and extract_path.exists():
        ...
        if meta.get("prompt_hash") == prompt_hash(namespace):
    ...
    if not text.strip():
        ...
        meta_path.write_text(
            json.dumps(
                {
                    "page_id": page_id,
                    "prompt_hash": prompt_hash(namespace),
                    "prompt_version": prompt_version(namespace),
                    "skipped": "empty",
                    "model": MODEL,
                },
                indent=2,
    ...
    for chunk_i, chunk in enumerate(chunks, start=1):
        ...
        subject = "Benchmark" if namespace == "eval" else "Product"
        focus = (
            "task and scoring claims"
            if namespace == "eval"
            else "memory-relevant claims"
        )
        user = (
            f"{subject}: {product_name} (id: {product_id}). "
            f"Extract only this {subject.lower()}.\n"
            f"Source kind: {kind}\n"
            f"Page URL: {url}\n"
            f"{kind_focus(kind, namespace=namespace)}\n"
            f"Extract {focus} from this text about {product_name} only. "
            "Respond with ONLY a JSON object, no markdown fences, no explanation.\n\n"
            f"{chunk}"
        )
        parsed = call_kiro(
            user,
            system_prompt(namespace)
...
def extract(
    ...
    *,
    ...
    with PipelineLogger(
        "extract", source=slug, pages=len(pages), force=force, namespace=namespace
    ) as log:
    ...
    if args.all:
        for slug in ids_for_namespace(namespace):
            ...
            if not (cache / "manifest.json").exists():
                print(f"extract {namespace} {slug}: skip (no harvest)")
                continue
            extract(slug, force=args.force, dry_run=args.dry_run, namespace=namespace)
        return
    if not args.slug:
